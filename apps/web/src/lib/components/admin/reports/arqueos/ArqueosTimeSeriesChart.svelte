<script lang="ts">
  /**
   * ArqueosTimeSeriesChart - Time series chart for arqueos discrepancies.
   * 
   * Displays system vs physical, differences, or discrepancies over time.
   * Responsive and accessible with reactive theme support.
   * Respects global module filter.
   */
  import { onMount } from 'svelte';
  import ChartWrapper from '../sales/ChartWrapper.svelte';
  import { createTimeSeriesConfig } from '$lib/utils/charts/chartConfig';
  import { transformArqueosTimeSeriesData, type ArqueosTimeSeriesDataPoint } from '$lib/utils/charts/chartHelpers';
  import {
    getChartThemeColors,
  ...
  export let data: ArqueosTimeSeriesDataPoint[] = [];
  export let metric: 'difference' | 'system' | 'physical' | 'discrepancies' = 'difference';
  ...
  $: {
    ...
    if (data && data.length > 0 && containerElement) {
      const transformed = transformArqueosTimeSeriesData(data, metric);
...
<div class="arqueos-time-series-chart" bind:this={containerElement}>
  {#if loading}
