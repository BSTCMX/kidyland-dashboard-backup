<script lang="ts">
  /**
   * SalesPieChart - Pie/Doughnut chart for sales data.
   * 
   * Displays revenue distribution by category (type, payment method, etc.).
  ...
  export let label: string = 'Distribución';
  ...
  // Reactive statement to update chart config when data, theme, or chartType changes
  $: {
...
<div class="pie-chart" bind:this={containerElement}>
  {#if loading}
