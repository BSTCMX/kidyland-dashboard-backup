<script lang="ts">
  /**
   * InventoryRecommendations - Actionable recommendations component.
   */
  import { onMount } from 'svelte';
  import { fetchInventoryRecommendations, type InventoryRecommendationsReport } from '$lib/stores/reports';
  ...
  let recommendationsData: InventoryRecommendationsReport | null = null;
  let loading = false;
  ...
  async function loadRecommendationsData() {
    ...
    try {
      const report = await fetchInventoryRecommendations(sucursalId);
    ...
    <div class="recommendations-container">
      ...
      {#if recommendationsData.recommendations.length === 0}
        <div class="no-recommendations">
          ...
          <p>No hay recomendaciones en este momento. El inventario está bien gestionado.</p>
        </div>
      {:else}
