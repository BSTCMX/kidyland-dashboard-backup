<script lang="ts">
  /**
   * PackageSaleForm component - Package sale form with step-by-step flow.
   * 
   * Handles complete package sale creation flow:
  ...
  // Date selection for package scheduling
  let selectedDate = "";
  ...
  $: selectedPackage = availablePackages.find((p) => p.id === selectedPackageId);
  $: totalCents =
    selectedPackage
      ? selectedPackage.price_cents * quantity - discountCents
      : 0;

  $: canProceedStep1 = selectedPackageId && quantity > 0 && selectedDate.trim() !== "";
  ...
  onMount(async () => {
    ...
    loading = true;
    try {
      await fetchAllPackages(currentUser.sucursal_id);
      // Filter to show only active service packages
      availablePackages = filterActiveServicePackages($packagesAdminStore.list);
  ...
  async function submitSale() {
    ...
    try {
      const formData = {
        ...
        quantity,
        scheduledDate: selectedDate || undefined, // Send selected date to backend
  ...
  function resetForm() {
    ...
    selectedPackageId = packageId || "";
    quantity = 1;
    selectedDate = today; // Reset to today
    childName = "";
    childAge = undefined;
    payerName = "";
    payerPhone = "";
        ...
        {#if selectedPackage}
          <div class="date-selector">
            ...
            <input
              ...
              required
            />
            <p class="help-text">Seleccione la fecha para la cual se programa el paquete</p>
          </div>
...
<style>
  ...
  .date-selector .input {
    ...
    border-radius: 12px;
    padding: 0 20px;
    padding-right: 50px; /* Extra space for calendar icon */
  ...
  /* Improve calendar picker indicator visibility and size */
  ...
  .date-selector .input::-webkit-calendar-picker-indicator:hover {
    opacity: 1;
    transform: scale(1.1);
  }

  /* For Firefox */
  .date-selector .input::-moz-calendar-picker-indicator {
    cursor: pointer;
    font-size: 22px;
    padding: 6px;
    margin-right: 4px;
  ...
  .date-selector .input::-moz-calendar-picker-indicator:hover {
    opacity: 1;
    transform: scale(1.1);
  }

  /* Note: Native date picker calendar popup size is controlled by the browser/OS.
     However, we can ensure the input is properly sized and styled.
     On mobile devices, the native calendar will use the full screen or a large modal,
     which provides a better UX than trying to force a specific size.
     
     For desktop browsers:
     - Chrome/Edge: Calendar popup size is controlled by browser
     - Firefox: Calendar popup size is controlled by browser
     - Safari: Calendar popup size is controlled by browser/OS
     
     The input styling above ensures good visibility and usability. */
  ...
  /* Botones con estilo Admin Forms - Sobrescribir estilos de Tailwind del componente Button */
  ...
  /* Estilos para variant="primary" - Sobrescribir Tailwind bg-blue-600 */
  ...
  .step-actions :global(button.hover\:bg-blue-700:hover:not(:disabled)) {
  ...
  .step-actions :global(button.bg-blue-600:active:not(:disabled)),
  .step-actions :global(button[class*="bg-blue"]:active:not(:disabled)) {
    background: linear-gradient(to bottom, #4a77d4, #3762bc) !important;
    background-color: #3762bc !important;
    box-shadow: inset 0 2px 4px rgba(0, 0, 0, 0.2) !important;
  }

  /* Estilos para variant="secondary" - Sobrescribir Tailwind bg-gray-200 */
  ...
  .step-actions :global(button.bg-gray-200:hover:not(:disabled)),
  .step-actions :global(button[class*="bg-gray"]:hover:not(:disabled)),
  .step-actions :global(button.hover\:bg-gray-300:hover:not(:disabled)) {
    background: linear-gradient(to bottom, #ffffff, #f5f5f5) !important;
    background-color: #f5f5f5 !important;
    border-color: #d4d4d4 !important;
  }

  .step-actions :global(button.bg-gray-200:active:not(:disabled)),
  .step-actions :global(button[class*="bg-gray"]:active:not(:disabled)) {
    background: linear-gradient(to bottom, #e6e6e6, #d4d4d4) !important;
