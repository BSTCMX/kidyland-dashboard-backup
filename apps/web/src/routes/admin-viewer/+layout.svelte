<script lang="ts">
  /**
   * Admin Viewer layout - Read-only layout for admin-viewer role.
   * 
   * Same structure as admin but with readonly indicators.
   * Only accessible to admin_viewer and super_admin.
   */
  ...
  // Reactive access check - uses secure checks that verify token-store consistency
  $: canAccess = $user && hasAccessSecure("/admin-viewer");
  $: adminPerms = $user ? getModulePermissions($user.role, "admin") : null;
  $: readonly = !adminPerms?.canEdit;
  ...
  onMount(() => {
    // Verify user has access to admin-viewer (backup check)
  ...
  // super_admin can return to /admin from /admin-viewer
  const rolesWithMainPanel: UserRole[] = ["super_admin"];
  ...
  $: currentPanelBase = currentPath.startsWith("/admin-viewer") ? "/admin-viewer" : currentPath;
  $: showReturnButton = 
    $user && 
    userMainPanelRoute &&
  ...
  const adminNavItems = [
    { route: "/admin-viewer", label: "Dashboard", icon: LayoutDashboard },
    { route: "/admin-viewer/users", label: "Usuarios", icon: Users },
    { route: "/admin-viewer/services", label: "Servicios", icon: Gamepad2 },
    { route: "/admin-viewer/products", label: "Productos", icon: ShoppingBag },
    { route: "/admin-viewer/reports", label: "Reportes", icon: TrendingUp },
  ];
</script>
  ...
  <div class="admin-layout">
    ...
    <aside class="sidebar" class:open={sidebarOpen}>
      ...
      <div class="sidebar-header">
        ...
        <h2 class="sidebar-title">Visualización</h2>
      </div>
      ...
      <!-- Return to Main Panel Button (for super_admin) -->
...
<style>
  ...
  .nav-link:hover .readonly-badge,
  .nav-link.active .readonly-badge {
    color: var(--accent-primary);
    opacity: 1;
  }
