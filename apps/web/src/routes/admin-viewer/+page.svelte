<script lang="ts">
  /**
   * Admin Viewer Dashboard - Read-only dashboard.
   * 
   * Same metrics as admin but without edit capabilities.
   */
  ...
  import ViewOnlyPanelSelector from "$lib/components/shared/ViewOnlyPanelSelector.svelte";
  ...
  import { metricsStore, formattedRevenue, formattedATV, timeSinceLastRefresh, refreshMetrics } from "$lib/stores/metrics";
  import { periodStore, initializePeriod } from "$lib/stores/period";
  import { 
    LayoutDashboard, 
    DollarSign, 
    Package, 
  ...
  onMount(() => {
    ...
    // This prevents race conditions where refreshMetrics() executes with stale token
  ...
  <div class="metrics-grid">
        ...
        {#if $metricsStore.peak_hours.peak_hours && $metricsStore.peak_hours.peak_hours.length > 0}
          <div class="metric-content">
            <div class="metric-value">
              {$metricsStore.peak_hours.busiest_hour.hour}:00h
            </div>
            <div class="metric-label">Hora más ocupada</div>
            
            <div class="metric-row">
              <span class="metric-label">Ventas en hora pico:</span>
              <span class="metric-value-small">{$metricsStore.peak_hours.busiest_hour.sales_count}</span>
            </div>
    ...
    {#if $metricsStore.top_products}
      <div class="metric-card">
        ...
        {#if $metricsStore.top_products.top_products && $metricsStore.top_products.top_products.length > 0}
          <div class="metric-content">
    ...
    {#if $metricsStore.top_services}
      <div class="metric-card">
        ...
        {#if $metricsStore.top_services.top_services && $metricsStore.top_services.top_services.length > 0}
          <div class="metric-content">
