<script lang="ts">
  /**
   * Monitor layout - Main layout for monitor routes.
   * 
   * Provides navigation sidebar and theme toggle.
   * Accessible to monitor, admin_viewer, super_admin roles.
   */
  ...
  $: canAccess = $user && hasAccessSecure("/monitor");
  ...
  onMount(() => {
    // Redirect monitor role to recepcion (monitor uses recepcion layout now)
    // This layout is only kept for /monitor/timers compatibility
    if ($user?.role === "monitor") {
      // If accessing /monitor root, redirect to recepcion
      // Allow /monitor/timers to continue working
  ...
  const monitorNavItems = [
    { route: "/monitor", label: "Dashboard", icon: LayoutDashboard },
  ...
  <div class="monitor-layout">
    ...
    <aside class="sidebar" class:open={sidebarOpen}>
      ...
      <div class="sidebar-header">
        ...
        <h2 class="sidebar-title">Monitor</h2>
      </div>

      <nav class="sidebar-nav">
        {#each monitorNavItems as item}
          <a
            href={item.route}
            class="nav-link"
            class:active={currentPath === item.route || (item.route !== "/monitor" && currentPath.startsWith(item.route))}
            on:click={closeSidebar}
          >
            <span class="nav-icon">
