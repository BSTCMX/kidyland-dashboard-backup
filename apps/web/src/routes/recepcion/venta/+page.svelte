<script lang="ts">
  /**
   * Venta page - Service sale form for reception.
   * 
   * Main page for creating new service sales.
   */
  import { onMount } from "svelte";
  import { goto } from "$app/navigation";
  import { user, hasAccessSecure, canEditSecure } from "$lib/stores/auth";
  import ServiceSaleForm from "$lib/components/forms/ServiceSaleForm.svelte";

  onMount(() => {
    // Verify user has access and can edit recepcion - uses secure checks that verify token-store consistency
...
<div class="venta-page">
  <div class="page-header">
    ...
    <h1 class="page-title">Nueva Venta - Servicio</h1>
  </div>
