<script lang="ts">
  /**
   * Reception layout - Main layout for reception routes.
   * 
   * Provides navigation sidebar and theme toggle.
   * Accessible to recepcion, admin_viewer, monitor roles.
   */
  ...
  $: canAccess = $user && hasAccessSecure("/recepcion");
  ...
  $: currentPanelBase = currentPath.startsWith("/recepcion") ? "/recepcion" : currentPath;
  $: showReturnButton = 
    $user && 
    userMainPanelRoute &&
  ...
  const receptionNavItems = [
    { route: "/recepcion", label: "Dashboard", icon: LayoutDashboard },
    { route: "/recepcion/servicios", label: "Servicios", icon: Gamepad2 },
    { route: "/recepcion/paquetes", label: "Paquetes", icon: Package },
    { route: "/recepcion/reportes", label: "Reportes", icon: FileText },
  ];
</script>
