<script lang="ts">
  /**
   * Venta page - Product package sale form for kidibar.
   * 
   * Main page for creating new product package sales with multi-selection.
   */
  import { onMount } from "svelte";
  import { goto } from "$app/navigation";
  import { user, hasAccessSecure, canEditSecure } from "$lib/stores/auth";
  import ProductPackageMultiSaleForm from "$lib/components/forms/ProductPackageMultiSaleForm.svelte";
...
<div class="venta-page">
  <div class="page-header">
    <button class="back-button" on:click={() => goto("/kidibar/paquetes")}>← Volver</button>
    <h1 class="page-title">Nueva Venta - Paquetes</h1>
  </div>

  <ProductPackageMultiSaleForm />
</div>
